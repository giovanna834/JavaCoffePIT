...
<html lang="en">
    ...
    <body>
        ...
        <div class="titulo-endereco">
            <p>Endereços <i class='bx bxs-edit-location' ></i></p>
        </div>
        ...
        <div class="row">
            <div class="col-8">
            <p>{{$endereco->logradouro}}, {{$endereco->numero}}<br>{{$endereco->cep}}, {{$endereco->cidade}} - {{$endereco->estado}} ({{$endereco->complemento}})</p>
            </div>
            <div class="col-2">
            <form action="/excluirendereco/{{$endereco->id}}" method="POST" class="botao-exend"> 
                @csrf
                @method('DELETE')
        ...
        <div class="botao">
            <a href="{{route('endereco')}}">Cadastrar novo endereço</a>
        </div>
